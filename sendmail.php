 ...
 */
function getTopText () {
    $message = "You received a new message sent via the contact form on lucianovelli.com.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on lucianovelli.com.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "lucianovelli.com"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 112.0.37",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "lucianovelli.com") {
...
function processMail() {
    ...
    if (empty($response)) {
        ...
        $contactFormEmails = json_decode('["info@lucianovelli.com"]');
        if (
